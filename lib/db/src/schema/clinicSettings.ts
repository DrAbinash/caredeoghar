...
export const clinicSettingsTable = pgTable("clinic_settings", {
  ...
  // Bill Print Settings (Premium A5 + Format controls)
  billDefaultFormat: text("bill_default_format").notNull().default("classic"),
  billClassicEnabled: boolean("bill_classic_enabled").notNull().default(true),
  billPremiumA5Enabled: boolean("bill_premium_a5_enabled").notNull().default(true),
  billDefaultCopyType: text("bill_default_copy_type").notNull().default("patient"),
  billShowAmountInWords: boolean("bill_show_amount_in_words").notNull().default(false),
  billShowSignatureLine: boolean("bill_show_signature_line").notNull().default(true),
  billShowComputerGenerated: boolean("bill_show_computer_generated").notNull().default(true),
  billShowReportMessage: boolean("bill_show_report_message").notNull().default(true),
  billShowServiceFooter: boolean("bill_show_service_footer").notNull().default(true),
  billShowBrandingFooter: boolean("bill_show_branding_footer").notNull().default(true),
  billDefaultPrintAction: text("bill_default_print_action").notNull().default("save-print"),
  billEnablePreview: boolean("bill_enable_preview").notNull().default(false),
  billDirectPrintAfterSave: boolean("bill_direct_print_after_save").notNull().default(true),
  billAutoOpenPrintDialog: boolean("bill_auto_open_print_dialog").notNull().default(true),
  billAskBeforePrint: boolean("bill_ask_before_print").notNull().default(false),
  billAutoDownloadPdf: boolean("bill_auto_download_pdf").notNull().default(false),
  billFastBillingMode: boolean("bill_fast_billing_mode").notNull().default(true),
  billAdminLock: boolean("bill_admin_lock").notNull().default(false),
