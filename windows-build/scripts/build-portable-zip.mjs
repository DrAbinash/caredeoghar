...
//
// Wraps windows-build/dist/payload/ + the compiled DiagnosticERP.exe into a
...
const STAGE   = path.join(BUILD_ROOT, "dist/portable/DiagnosticERP");
const OUT_ZIP = path.join(BUILD_ROOT, "dist/DiagnosticERP-Portable.zip");
...
async function main() {
  ...
  await run("zip", ["-rq", OUT_ZIP, "DiagnosticERP"], { cwd: path.dirname(STAGE) });
