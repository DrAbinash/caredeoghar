...
const ZIP_FILE   = path.join(DIST_DIR, "CareDiagnostics-Desktop-win-unpacked.zip");
const NSI_FILE   = path.join(BUILD_ROOT, "installer/installer-electron.nsi");
const OUT_FILE   = path.join(DIST_DIR, "CareDiagnostics-Desktop-Setup.exe");
...
async function main() {
  ...
  // ---- Phase 1: electron-builder --win dir -------------------------------
  if (await pathExists(path.join(UNPACKED, "Care Diagnostics Desktop.exe"))) {
