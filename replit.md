...
-   **Stale composite `dist/` shadows fresh codegen output.** `lib/api-client-react` and `lib/api-zod` are composite TS projects that emit `dist/*.d.ts`. After re-running `pnpm --filter @workspace/api-spec run codegen`, leftover `dist/` from a previous `tsc --build` can shadow the freshly regenerated `.ts` source and produce phantom TS2339 / TS6305 errors. The `codegen` npm script now runs `lib/api-spec/scripts/clean-stale-dist.mjs` automatically to wipe those `dist/` directories. If you add another composite lib that consumes the OpenAPI output, append its `dist` path to that script's `targets` list.
