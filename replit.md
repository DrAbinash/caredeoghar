...
The super-admin surface is gated by a physical USB pen drive containing `superadmin.key`, whose content must match the `SUPER_ADMIN_USB_KEY` environment secret. Implemented in `artifacts/api-server/src/middleware/requireSuperAdminUsb.ts` and applied to `POST /api/super-admin/login` plus all `requireSuperAdmin`-protected routes (defense in depth). Public endpoints `GET /api/super-admin/usb/status` and `POST /api/super-admin/usb/verify` (rate-limited) let the UI verify a key file. The Super Admin link in the billing UI sidebar (`artifacts/diagnostic-erp/src/components/Layout.tsx`) is invisible until the user picks the key file via the "Insert USB key" affordance; the super-admin-portal also shows a `UsbUnlockScreen` before its PIN login. Key is stashed in `sessionStorage` (dies with the tab). When `SUPER_ADMIN_USB_KEY` is unset, the gate is bypassed with a startup warning (back-compat). See `PEN_DRIVE_SETUP.md` for operator instructions.
