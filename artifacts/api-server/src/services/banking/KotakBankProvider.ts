...
// KotakBankProvider
//
// Adapter for Kotak Mahindra Bank API.
// Credentials: KOTAK_API_KEY, KOTAK_API_SECRET, KOTAK_CLIENT_ID, KOTAK_WEBHOOK_SECRET
// =============================================================================
...
export class KotakBankProvider implements BankProvider {
  readonly name = "kotak";

  private creds: ProviderCredentials = {};
  private baseUrl = "https://developer.kotak.com/api/v1";
  ...
  private async api<T>(path: string, opts?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`Kotak API ${res.status}: ${body}`);
    }
  ...
  async getTransactions(accountId: string, options?: { fromDate?: Date; toDate?: Date; limit?: number }): Promise<BankTransaction[]> {
    ...
    const data = await this.api<{
      transactions: Array<{
        ...
        drCr: string; closingBalance: string; utr?: string; refNo?: string;
      }>;
    ...
    return (data.transactions ?? []).map((t) => ({
      ...
      type: t.drCr === "CR" ? "credit" : "debit",
      balanceAfter: parseFloat(t.closingBalance) || undefined,
      utr: t.utr,
  ...
  async parseWebhookPayload(rawBody: string): Promise<WebhookPayload> {
    ...
    return { eventType: parsed.eventType || parsed.type || "kotak.unknown", data: parsed };
  }
  ...
  async initiatePayment(accountId: string, payment: PaymentInitiation): Promise<PaymentResult> {
    const data = await this.api<{
      ...
      body: JSON.stringify({
        ...
        payeeAccount: payment.beneficiaryAccount,
        payeeIfsc: payment.beneficiaryIfsc,
      }),
    });
    return {
