...
// HDFCBankProvider
//
// Adapter for HDFC Bank Corporate API.
// Credentials: HDFC_API_KEY, HDFC_API_SECRET, HDFC_CLIENT_ID, HDFC_WEBHOOK_SECRET
// =============================================================================
...
export class HDFCBankProvider implements BankProvider {
  readonly name = "hdfc";

  private creds: ProviderCredentials = {};
  private baseUrl = "https://developer.hdfcbank.com/api/v1";
  ...
  private async api<T>(path: string, opts?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`HDFC API ${res.status}: ${body}`);
    }
  ...
  async getTransactions(accountId: string, options?: { fromDate?: Date; toDate?: Date; limit?: number }): Promise<BankTransaction[]> {
    ...
    const data = await this.api<{
      transactions: Array<{
        txnId: string; txnDate: string; narration: string; amount: string;
        crDr: string; closingBalance: string; utrNo?: string; refNo?: string;
      }>;
  ...
  async parseWebhookPayload(rawBody: string): Promise<WebhookPayload> {
    ...
    return { eventType: parsed.eventType || parsed.type || "hdfc.unknown", data: parsed };
  }
