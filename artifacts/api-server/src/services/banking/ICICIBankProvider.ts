...
// ICICIBankProvider
//
// Adapter for ICICI Bank API.
// Credentials: ICICI_API_KEY, ICICI_API_SECRET, ICICI_CLIENT_ID, ICICI_WEBHOOK_SECRET
// =============================================================================
...
export class ICICIBankProvider implements BankProvider {
  readonly name = "icici";

  private creds: ProviderCredentials = {};
  private baseUrl = "https://developer.icicibank.com/api/v1";
  ...
  private async api<T>(path: string, opts?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`ICICI API ${res.status}: ${body}`);
    }
  ...
  async parseWebhookPayload(rawBody: string): Promise<WebhookPayload> {
    ...
    return { eventType: parsed.eventType || parsed.type || "icici.unknown", data: parsed };
  }
