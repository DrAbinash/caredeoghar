...
// SBIBankProvider
//
// Adapter for State Bank of India API.
// Credentials: SBI_API_KEY, SBI_API_SECRET, SBI_CLIENT_ID, SBI_WEBHOOK_SECRET
// =============================================================================
...
export class SBIBankProvider implements BankProvider {
  readonly name = "sbi";

  private creds: ProviderCredentials = {};
  private baseUrl = "https://developer.sbi.co.in/api/v1";
  ...
  private async api<T>(path: string, opts?: RequestInit): Promise<T> {
    ...
    if (!res.ok) {
      ...
      throw new Error(`SBI API ${res.status}: ${body}`);
    }
  ...
  async parseWebhookPayload(rawBody: string): Promise<WebhookPayload> {
    ...
    return { eventType: parsed.eventType || parsed.type || "sbi.unknown", data: parsed };
  }
